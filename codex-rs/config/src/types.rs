...
pub struct Notice {
    ...
    /// Tracks whether the user opted out of the safety-buffering retry menu.
    /// The menu itself says no action is required, and the status line carries
    /// the same message, so hiding it costs no information.
    pub hide_safety_buffering_retry_prompt: Option<bool>,
